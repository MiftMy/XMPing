...
@implementation XMPing
#pragma mark - public
+ (XMPing *)simplePingWithHostName:(NSString *)hostName
// See comment in header.
{
    return [[XMPing alloc] initWithHostName:hostName address:nil];
}

+ (XMPing *)simplePingWithHostAddress:(NSData *)hostAddress
// See comment in header.
{
    return [[XMPing alloc] initWithHostName:nil address:hostAddress];
}
#pragma mark - private
...
// Shut down the pinger object and tell the delegate about the error.
{
    ...
    [self stop];
    if ( (self.delegate != nil) && [self.delegate respondsToSelector:@selector(XMPing:didFailWithError:)] ) {
        [self.delegate XMPing:self didFailWithError:error];
    }
}
...
// See comment in header.
{
    ...
    if ( (bytesSent > 0) && (((NSUInteger) bytesSent) == [packet length]) ) {
        ...
        if ( (self.delegate != nil) && [self.delegate respondsToSelector:@selector(XMPing:didSendPacket:)] ) {
            [self.delegate XMPing:self didSendPacket:packet];
        }
    } else {
        NSError *   error;
        ...
        if ( (self.delegate != nil) && [self.delegate respondsToSelector:@selector(XMPing:didFailToSendPacket:error:)] ) {
            [self.delegate XMPing:self didFailToSendPacket:packet error:error];
        }
    }
...
// messages waiting on the socket.
{
    ...
    if (bytesRead > 0) {
        ...
        if ( [self _isValidPingResponsePacket:packet] ) {
            if ( (self.delegate != nil) && [self.delegate respondsToSelector:@selector(XMPing:didReceivePingResponsePacket:)] ) {
                [self.delegate XMPing:self didReceivePingResponsePacket:packet];
            }
        } else {
            if ( (self.delegate != nil) && [self.delegate respondsToSelector:@selector(XMPing:didReceiveUnexpectedPacket:)] ) {
                [self.delegate XMPing:self didReceiveUnexpectedPacket:packet];
            }
        }
    } else {
...
// ICMP socket.  It just redirects the call to Objective-C code.
{
    ...
    obj = (__bridge XMPing *) info;
    assert([obj isKindOfClass:[XMPing class]]);
...
// We have a host address, so let's actually start pinging it.
{
    ...
    if (err != 0) {
        ...
        if ( (self.delegate != nil) && [self.delegate respondsToSelector:@selector(XMPing:didStartWithAddress:)] ) {
            [self.delegate XMPing:self didStartWithAddress:self.hostAddress];
        }
    }
    assert(fd == -1);
}
